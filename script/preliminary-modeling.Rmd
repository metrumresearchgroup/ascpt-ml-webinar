...
dat <- dat_raw %>%
  mutate(class_n = Class == "recurrence-events",
         ...
         case_weight = importance_weights(if_else(class_n == 0, 1, 201/85)) # For later
  )
...
recipie_xgboost_tidymodels_weights <- recipes::recipe(
  Class ~age_f + menopause + tumor_size_f + inv_nodes_f + node_caps_n + 
...
xgb_wf_weights <- xgb_wf %>%
  update_recipe(recipie_xgboost_tidymodels_weights) %>%
...
p <- xgb.ggplot.shap.summary(data = bake(extract_recipe(final_fit), testing(init_split)) %>% select(-Class) %>% as.matrix(),
...
Check tumor size makes sense with the predictions
