...
Data source: https://archive.ics.uci.edu/ml/datasets/breast+cancer. Class is the dependent variable, and the goal is to classify patients based on the other features (covariates). 
...
fit1 <- xgboost::xgboost(data = ,### FILL THIS IN ###  # predictors
                         label = ,### FILL THIS IN ### # responses  
...
cv_folds <- vfold_cv(training(init_split), 
...
# To help answer this, build a confusion matrix for this model and the null model (i.e. always predict no recurrence).
...
### CREATE CONFUSION MATRIX FOR NAIVE MODEL ### 
...
### EXERCISE: Output the first 3 trees. ### 
# How do you interpret this sequence of trees?
```
...
# As before, use cross validation to tune the model hyperparameters
...
best_auc_weighted <- select_best(tuning_results_weights,
                                 metric = ### FILL THIS IN ###
)
...
# TODO: Caclulate the precision-recall AUC
